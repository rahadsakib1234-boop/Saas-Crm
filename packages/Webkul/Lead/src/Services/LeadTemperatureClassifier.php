...
class LeadTemperatureClassifier
{
    ...
    public function classify(Lead $lead): array
    {
        ...
        // Step 4: Determine threshold and execute actions
        ...
        $results = $this->executor->executeAll($threshold['actions'] ?? [], $lead, $log);

        // Step 6: Record guard hits (only after successful execution)
        ...
        $hasFailure = collect($results)->contains('error');
        $log->markSuccess();
    ...
    protected function createLog(Lead $lead, array $guardFailures = []): LeadAutomationLog
    {
        ...
        return \Webkul\Lead\Models\LeadAutomationLog::create([
            'lead_id' => $lead->id,
