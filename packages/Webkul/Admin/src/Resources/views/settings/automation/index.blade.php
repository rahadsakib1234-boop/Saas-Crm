<x-admin::layouts>
    <!-- Page Title -->
    <x-slot:title>
        @lang('admin::app.settings.automation.index.title')
    </x-slot>
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="scroll-reactive-sticky sticky top-[60px] z-[1000] flex items-center justify-between rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
            <div class="flex flex-col gap-2">
                {!! view_render_event('admin.settings.automation.index.breadcrumbs.before') !!}
                <x-admin::breadcrumbs name="settings.automation" />
                {!! view_render_event('admin.settings.automation.index.breadcrumbs.after') !!}
            ...
            <div class="flex items-center gap-x-2.5">
                @if (bouncer()->hasPermission('settings.automation.rules.create'))
                    <a
                        href="{{ route('admin.settings.automation.rules.create') }}"
                        class="primary-button"
                    >
                        @lang('admin::app.settings.automation.index.create-btn')
                    </a>
                @endif
            </div>
        </div>
        ...
        <!-- Rules DataGrid -->
        <x-admin::datagrid :src="route('admin.settings.automation.rules.index')">
